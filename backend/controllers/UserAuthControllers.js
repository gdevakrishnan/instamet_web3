...
const getAUserDetails = async (req, res) => {
    ...
    try {
        ...
        const isPasswordMatched = await bcrypt.compare(pwd, task.pwd);
        if (task) {
...
const verify_token = async (req, res) => {
    try {
        ...
        const response = decode.userData;
        res.status(200).json({ ...response, uname: decode.userData.uname });
    } catch (e) {
        res.status(400).json({ message: e.message });
    }
}

module.exports = { addUserDetails, getAUserDetails, updateUserDetails, deleteUserDetails, verify_token };